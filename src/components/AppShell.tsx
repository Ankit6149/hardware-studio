...
function renderSurface(surface: NavigationSurface, viewId: string): React.ReactNode {
  ...
  switch (surface) {
    ...
    case 'firmware-state-machine': return <EngineeringFirmwareWorkbench initialMode="state-machine" />;
    case 'firmware-hardware-map': return <EngineeringFirmwareWorkbench initialMode="hardware-map" />;
    case 'firmware-source': return <EngineeringFirmwareWorkbench initialMode="source" />;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <button type="button" onClick={onReturn} className="mt-4 inline-flex min-h-9 items-center gap-2 bg-slate-950 px-3 text-xs font-semibold text-white hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-500/70 focus:ring-offset-1">
            <ArrowLeft className="h-3.5 w-3.5" aria-hidden="true" /> Return to project home
          </button>
        </div>
      </div>
    </div>
