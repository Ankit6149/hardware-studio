...
const guides: Record<string, WorkspaceGuide> = {
  ...
  'product-design': {
    ...
    purpose: 'Explore product form and intent without pretending concept geometry is manufacturing geometry.',
    objectModel: PRODUCT_MODEL,
    steps: [
      { title: 'Choose the scope', detail: 'Decide whether the concept represents the whole product, one assembly or one enclosure.' },
      { title: 'Add references and concept parts', detail: 'Use visual layers to communicate form, packaging and relative arrangement.' },
      { title: 'Capture intent dimensions', detail: 'Record dimensions that matter so Mechanical can turn them into physical constraints.' },
      { title: 'Hand off deliberately', detail: 'Move confirmed intent into Requirements, Mechanical or Electronics instead of duplicating it.' },
    ],
    next: 'Do not manufacture from concept geometry; move confirmed physical intent into Mechanical.',
  },
  requirements: {
    title: 'Requirements',
    ...
    steps: [
      ...
      { title: 'Link responsibility', detail: 'Connect the requirement to the assembly, board, component, firmware module or validation work that owns it.' },
    ],
  ...
  'product-architecture': {
    ...
    objectModel: PRODUCT_MODEL,
    steps: [
      { title: 'Create functional blocks', detail: 'Use real responsibilities such as sensing, power, motion, compute, enclosure or communications.' },
    ...
    next: 'Use architecture to decide where detailed work belongs.',
  },
  'schematic-editor': {
    ...
    steps: [
      ...
      { title: 'Wire pins', detail: 'Use Wire (W), click a real source pin, route the connection, then finish on the destination pin.' },
      { title: 'Inspect ERC', detail: 'Review missing or conflicting electrical connections before moving to PCB.' },
    ],
  ...
  'board-designer': {
    ...
    steps: [
      ...
      { title: 'Place footprints', detail: 'Open Browser and drag project components onto the board. Their pads are routing anchors.' },
      ...
      { title: 'Run DRC', detail: 'Resolve clearance, placement and connectivity findings before release.' },
    ],
  ...
  'mechanical-studio': {
    ...
    steps: [
      { title: 'Choose the physical scope', detail: 'Decide whether this represents the whole enclosure or one assembly inside a larger machine.' },
      { title: 'Link PCB reference when relevant', detail: 'Bring in the authoritative board envelope so openings and clearances reference real PCB geometry.' },
      { title: 'Create named physical features', detail: 'Use Outer Profile, Connector Opening, Mounting Point, Battery Cavity or keepouts—not anonymous rectangles.' },
      { title: 'Dimension and tolerance', detail: 'Select a feature and add the dimensions manufacturing or assembly depends on.' },
    ],
    next: 'If a shape has no engineering meaning, name its purpose before adding more geometry.',
  },
  'assembly-stack': {
    title: 'Assembly',
    purpose: 'Describe physical build order, materials, fastening and inspection for one assembly.',
  ...
  'firmware-studio': {
    ...
    purpose: 'Break software behavior into responsibilities linked to real hardware and evidence.',
    ...
    steps: [
      ...
      { title: 'Map hardware', detail: 'Link each module to the components, pins and nets it controls or observes.' },
      { title: 'Implement in Source', detail: 'Create or import real source files and link them to modules.' },
      { title: 'Record evidence', detail: 'Verification requires a successful build record plus a passing device observation.' },
    ],
  ...
  'state-machines': {
    ...
    steps: [
      { title: 'Create meaningful states', detail: 'Use product modes such as Boot, Idle, Measuring, Charging or Fault.' },
      { title: 'Add transitions', detail: 'Every transition should have a real event or condition that causes it.' },
      { title: 'Validate behavior', detail: 'Resolve unreachable or ambiguous states before implementation.' },
    ],
  ...
  'hardware-mapping': {
    ...
    steps: [
      ...
      { title: 'Link physical components', detail: 'Map the sensors, controllers, drivers or interfaces used by that module.' },
      { title: 'Map pins and nets', detail: 'Capture the electrical contract the source code must honor.' },
    ],
  ...
  'source-skeleton': {
    ...
    steps: [
      ...
      { title: 'Edit and save', detail: 'Use Explorer to switch files. Ctrl/Cmd + S saves the active file.' },
      { title: 'Link source to a module', detail: 'A file becomes useful evidence when connected to the module it implements.' },
      { title: 'Build and observe', detail: 'Record the real toolchain result and device observation in Build & Device Evidence.' },
    ],
    next: 'Source is implementation; build + device observation is verification.',
  },
  'validation-studio': {
    title: 'Validation',
...
export const WorkspaceCoach: React.FC<{ viewId: string }> = ({ viewId }) => {
  const guide = resolveGuide(viewId);
  ...
  const open = forcedOpen || (!persistedDismissed && !dismissedThisSession);
  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setForcedOpen(true)} className="absolute bottom-9 right-3 z-40 inline-flex h-9 items-center gap-2 border border-slate-300 bg-[#fbfaf6] px-3 text-[11px] font-semibold text-slate-700 shadow-[0_8px_22px_rgba(17,17,15,0.10)] hover:bg-white focus:outline-none focus:ring-2 focus:ring-slate-400" aria-label={`Open ${guide.title} guide`}>
  ...
  return (
    <aside className="absolute bottom-9 right-3 z-40 w-[360px] max-w-[calc(100%-1.5rem)] overflow-hidden border border-slate-300 bg-[#fbfaf6] shadow-[0_18px_48px_rgba(17,17,15,0.16)]" aria-label={`${guide.title} guide`}>
      <div className="flex items-start gap-3 border-b border-slate-300 bg-[#f2efe7] px-4 py-3">
        ...
        <div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">How this workspace works</p><h2 className="mt-0.5 text-[14px] font-semibold tracking-tight text-slate-950">{guide.title}</h2></div>
      ...
      <div className="max-h-[60vh] overflow-y-auto px-4 py-3">
        ...
        <div className="mt-3 border border-slate-200 bg-white px-3 py-2.5"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-500"><Layers3 className="h-3.5 w-3.5" /> Mental model</div><p className="mt-1.5 text-[11px] leading-5 text-slate-700">{guide.objectModel}</p></div>
        <ol className="mt-3 space-y-2">{guide.steps.map((step, index) => <li key={step.title} className="grid grid-cols-[1.5rem_minmax(0,1fr)] gap-2.5"><span className="mt-0.5 grid h-5 w-5 place-items-center border border-slate-300 bg-white text-[9px] font-semibold tabular-nums text-slate-700">{index + 1}</span><div><p className="text-[11px] font-semibold text-slate-900">{step.title}</p><p className="mt-0.5 text-[10px] leading-4 text-slate-500">{step.detail}</p></div></li>)}</ol>
        <div className="mt-3 flex items-start gap-2 border-t border-slate-200 pt-3"><ChevronRight className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-500" aria-hidden="true" /><p className="text-[10px] leading-4 text-slate-600"><strong className="font-semibold text-slate-900">Next:</strong> {guide.next}</p></div>
      </div>
    </aside>
  );
};
