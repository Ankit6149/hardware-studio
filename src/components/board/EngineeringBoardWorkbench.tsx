...
const ROUTING_LAYERS = [
  { id: 'top-copper', label: 'Top copper' },
  { id: 'bottom-copper', label: 'Bottom copper' },
  { id: 'silkscreen', label: 'Silkscreen' },
  { id: 'drill', label: 'Drill / vias' },
  { id: 'keepouts', label: 'Keepouts' },
  { id: 'ratsnest', label: 'Ratsnest' },
] as const;
...
export const EngineeringBoardWorkbench: React.FC = () => {
  ...
  const boardVias = activeBoard ? vias.filter((via) => via.boardId === activeBoard.id) : [];

  const widthRule = pcbRules.find((rule) => rule.boardId === activeBoard?.id && /(?:track|trace).*width/i.test(rule.ruleType));
  const routeWidthMm = numericRuleValue(widthRule?.value) || 0.25;
  const routeWidthIsFallback = !numericRuleValue(widthRule?.value);
  ...
  const netRows = useMemo(() => nets.map((net) => {
  ...
  }), [boardTraces, components, nets, padNetAssignments]);
  ...
  const selectObject = (componentId: string) => {
  ...
  if (!outline) {
    return (
      <section className="flex h-full min-h-0 flex-col bg-[#f7f5ef]">
        ...
        <div className="grid min-h-0 flex-1 place-items-center bg-white p-6"><div className="max-w-lg text-center"><CircuitBoard className="mx-auto h-8 w-8 text-slate-300" /><h1 className="mt-3 text-base font-semibold text-slate-950">Define the board outline first</h1><p className="mt-1 text-xs leading-5 text-slate-500">The editing canvas stays blocked until the board has explicit physical geometry. No hidden 50 × 30 mm fallback is used as design truth.</p></div></div>
      </section>
    );
  }
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden bg-[#f7f5ef] text-slate-900 hs-engineering-pcb" aria-label="PCB engineering workbench">
      <EngineeringEditorBar
        domain="Electronics"
        title="PCB Layout"
        ...
        docks={(
          <>
            ...
            <EditorDockButton label="Inspector" icon={PanelRight} active={inspectorOpen} count={hardDrcCount} onClick={() => setInspectorOpen((value) => !value)} />
          </>
        )}
        actions={(
          <>
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden bg-white">
              ...
              <div className="p-2">
                ...
                <div className="space-y-1">
                  {unplaced.map((component) => (
                    <div key={component.id} draggable onDragStart={(event) => { event.dataTransfer.setData('application/hardware-studio-component', component.id); selectObject(component.id); }} className="flex cursor-grab items-center gap-2 border border-slate-200 bg-white px-2 py-1.5 active:cursor-grabbing">
                      <div className="min-w-0 flex-1"><p className="truncate text-[10px] font-semibold text-slate-900">{component.referenceDesignator} · {component.componentName}</p><p className="truncate font-mono text-[8px] text-slate-400">{component.footprint || 'footprint unresolved'}</p></div><span className="text-[8px] text-slate-400">drag to board</span>
                    </div>
                  ))}
                ...
                <div className="space-y-0.5">{placed.map((component) => <button key={component.id} type="button" onClick={() => selectObject(component.id)} className={`flex w-full items-center gap-2 px-2 py-1.5 text-left ${viewState.selectedComponentId === component.id ? 'bg-slate-950 text-white' : 'hover:bg-slate-100'}`}><span className="min-w-0 flex-1 truncate text-[10px] font-semibold">{component.referenceDesignator} · {component.componentName}</span><span className={`font-mono text-[8px] ${viewState.selectedComponentId === component.id ? 'text-white/60' : 'text-slate-400'}`}>{component.placementX?.toFixed(2)}, {component.placementY?.toFixed(2)}</span></button>)}</div>
              </div>
            ) : (
              <div className="p-2">
                {ROUTING_LAYERS.map((layer) => {
                  ...
                  return <div key={layer.id} className={`flex min-h-9 items-center gap-2 px-1.5 ${active ? 'bg-slate-100' : ''}`}><button type="button" onClick={() => updateView({ layerVisibility: { ...viewState.layerVisibility, [layer.id]: !visible }, showRatsnest: layer.id === 'ratsnest' ? !visible : viewState.showRatsnest })} className="grid h-7 w-7 place-items-center text-slate-400 hover:text-slate-900">{visible ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}</button><button type="button" onClick={() => updateView({ activeLayerId: layer.id })} className={`min-w-0 flex-1 truncate text-left text-[10px] ${active ? 'font-semibold text-slate-950' : 'text-slate-600'}`}>{layer.label}</button></div>;
                })}
                <div className="mt-3 border-t border-slate-200 pt-3 text-[9px] leading-4 text-slate-500">Copper layers here control the editor view. Manufacturing stackup remains defined by the project’s persisted PCB layer data.</div>
              </div>
            )}
          </EngineeringDock>
        )}
        ...
        {inspectorOpen && (
          <EngineeringDock side="right" title="Inspector" subtitle="Selection, nets and design rules" onClose={() => setInspectorOpen(false)} widthClassName="w-[320px]">
            <div className="flex border-b border-slate-200 bg-white p-1">
              {([
                ['selection', 'Selection', PanelRight],
                ['nets', 'Nets', Network],
                ['drc', 'DRC', AlertTriangle],
              ] as const).map(([id, label, Icon]) => <button key={id} type="button" onClick={() => setInspectorTab(id)} className={`h-7 flex-1 text-[9px] font-semibold ${inspectorTab === id ? 'bg-slate-950 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><Icon className="mr-1 inline h-3 w-3" />{label}</button>)}
            </div>
            {inspectorTab === 'selection' ? (
              <div className="p-3">
                {selectedComponent ? (
                  <div className="space-y-3"><div><p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-slate-400">Footprint</p><p className="mt-1 text-[12px] font-semibold text-slate-950">{selectedComponent.referenceDesignator} · {selectedComponent.componentName}</p><p className="mt-0.5 font-mono text-[9px] text-slate-500">{selectedComponent.footprint}</p></div><div className="grid grid-cols-2 gap-2"><label className="text-[9px] text-slate-500">X mm<input type="number" step={viewState.gridSizeMm} value={selectedComponent.placementX ?? ''} onChange={(event) => updatePCBPlacement(selectedComponent.id, { placementX: Number.parseFloat(event.target.value) })} className="mt-1 h-8 w-full border border-slate-300 bg-white px-2 font-mono text-[10px] text-slate-900" /></label><label className="text-[9px] text-slate-500">Y mm<input type="number" step={viewState.gridSizeMm} value={selectedComponent.placementY ?? ''} onChange={(event) => updatePCBPlacement(selectedComponent.id, { placementY: Number.parseFloat(event.target.value) })} className="mt-1 h-8 w-full border border-slate-300 bg-white px-2 font-mono text-[10px] text-slate-900" /></label></div><label className="block text-[9px] text-slate-500">Rotation<input type="number" value={selectedComponent.rotationDeg || 0} onChange={(event) => updatePCBPlacement(selectedComponent.id, { rotationDeg: Number.parseFloat(event.target.value) || 0 })} className="mt-1 h-8 w-full border border-slate-300 bg-white px-2 font-mono text-[10px] text-slate-900" /></label><div className="grid grid-cols-2 gap-px border border-slate-200 bg-slate-200 text-[9px]">{[['Side', selectedComponent.side || 'Top'], ['Pads', String(getFootprint(selectedComponent.footprint).pads.length)], ['Criticality', selectedComponent.placementCriticality], ['Status', selectedComponent.placementStatus || 'Placed']].map(([label, value]) => <div key={label} className="bg-white p-2"><p className="text-slate-400">{label}</p><p className="mt-0.5 truncate font-semibold text-slate-800">{value}</p></div>)}</div><div className="flex gap-1"><button type="button" onClick={rotateSelected} className="h-8 flex-1 border border-slate-300 bg-white text-[9px] font-semibold text-slate-700">Rotate 90°</button><button type="button" onClick={() => updateBoardComponent(selectedComponent.id, { side: selectedComponent.side === 'Bottom' ? 'Top' : 'Bottom' })} className="h-8 flex-1 border border-slate-300 bg-white text-[9px] font-semibold text-slate-700">Flip side</button></div></div>
                ) : selectedTrace ? (
                  <div className="space-y-3"><p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-slate-400">Trace</p><p className="text-[12px] font-semibold text-slate-950">{selectedTrace.netName || 'Unnamed net'}</p><label className="block text-[9px] text-slate-500">Width mm<input type="number" step="0.05" value={selectedTrace.width || 0.25} onChange={(event) => updateTrace(selectedTrace.id, { width: Number.parseFloat(event.target.value) || 0.25 })} className="mt-1 h-8 w-full border border-slate-300 bg-white px-2 font-mono text-[10px]" /></label><p className="text-[9px] text-slate-500">{selectedTrace.layerId || 'top-copper'} · {selectedTrace.points?.length || 0} vertices · {selectedTrace.status || 'Draft'}</p></div>
                ) : selectedVia ? (
                  <div><p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-slate-400">Via</p><p className="mt-1 font-mono text-[10px] text-slate-700">{selectedVia.x?.toFixed(2)}, {selectedVia.y?.toFixed(2)} mm</p><p className="mt-2 text-[9px] text-slate-500">Ø {selectedVia.outerDiameter || 0.6} / drill {selectedVia.drillDiameter || 0.3} mm</p></div>
                ) : <p className="text-[10px] leading-5 text-slate-500">Select a footprint, trace, or via. The inspector edits that engineering object without changing workspaces.</p>}
              </div>
            ) : inspectorTab === 'nets' ? (
              <div className="p-2"><p className="px-1 pb-1.5 text-[9px] text-slate-500">Choose a net to arm the router. Routing still starts from a real pad, via, or trace endpoint.</p><div className="space-y-0.5">{netRows.map((net) => <button key={net.id} type="button" onClick={() => { updateView({ selectedNetName: net.netName, activeTool: 'route' }); setInspectorOpen(false); }} className={`flex w-full items-center gap-2 px-2 py-1.5 text-left ${viewState.selectedNetName === net.netName ? 'bg-slate-950 text-white' : 'hover:bg-slate-100'}`}><span className="min-w-0 flex-1 truncate text-[10px] font-semibold">{net.netName}</span><span className={`font-mono text-[8px] ${viewState.selectedNetName === net.netName ? 'text-white/60' : 'text-slate-400'}`}>{net.padCount} pads · {net.traceCount} traces</span></button>)}</div></div>
            ) : (
              <div className="p-2"><div className="mb-2 flex items-center justify-between px-1"><span className="text-[9px] text-slate-500">Current board findings</span><button type="button" onClick={runDrc} className="h-7 border border-slate-300 bg-white px-2 text-[9px] font-semibold text-slate-700">Run again</button></div><div className="space-y-1">{drcResults.map((result) => <button key={result.id} type="button" onClick={() => { if (result.linkedObjectType === 'component') selectObject(result.linkedObjectId); }} className="w-full border-l-2 border-amber-500 bg-amber-50 px-2 py-1.5 text-left"><p className="text-[9px] font-semibold text-amber-950">{result.severity} · {result.title}</p><p className="mt-0.5 text-[9px] leading-4 text-amber-800">{result.description}</p></button>)}{drcResults.length === 0 && <p className="p-3 text-[9px] text-emerald-700">No current DRC findings.</p>}</div></div>
            )}
          </EngineeringDock>
        )}
      </div>
      ...
      <EngineeringStatusBar
        ...
        center={`${viewState.activeLayerId} · width ${routeWidthMm.toFixed(2)} mm${routeWidthIsFallback ? ' fallback (no explicit width rule)' : ''}`}
